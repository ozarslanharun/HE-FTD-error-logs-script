...
parser.add_argument(
    "--builds", type=int, default=1, help="The number of builds to run."
)
parser.add_argument(
    "--parallel", type=int, default=1, help="The maximum number of worker threads."
)
parser.add_argument(
    "--test_name",
    type=str,
    default=f"atxHYE_flakyTest_{formatted_current_time}",
    help="The base name of the test.",
)
parser.add_argument(
    "--build_name",
    type=str,
    default=f"atxHYE_flakyBuild_{formatted_current_time}",
    help="The base name of the build.",
)
parser.add_argument(
    "--username",
    type=str,
    default=os.environ.get("LT_USERNAME"),
    help="The username to use for the selenium tests.",
)
parser.add_argument(
    "--access_key",
    type=str,
    default=os.environ.get("LT_ACCESS_KEY"),
    help="The access key to use for the selenium tests.",
)
parser.add_argument(
    "--env",
    type=str,
    default="prod",
    choices=["stage", "prod"],
    help="The environment to run the tests in. Default is stage.",
)
parser.add_argument(
    "--hub_url",
    type=str,
    default="hub",
    help="The hub url to use for the selenium tests.",
)
...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="To increase command count in the test by factor of 8",
)
parser.add_argument(
    "--error_commands",
    type=int,
    default=0,
    help="To include error commands in the test, maximum value that can be passed is 15",
)
parser.add_argument(
    "--error_repeat",
    type=int,
    default=1,
    help="To run the chosen error n number of times",
)
parser.add_argument(
    "--build_tags",
    type=str,
    default="he_ftd_build_tag",
    help="Build tags to include in the test. Comma separated.",
)
parser.add_argument(
    "--test_tags",
    type=str,
    default="he_ftd_test_tag",
    help="Test tags to include in the test. Comma separated.",
)
parser.add_argument(
    "--caps_json", type=str, default="capsV3.json", help="Path to the caps json file."
)
parser.add_argument(
    "--project",
    type=str,
    default="ATXproject",
    help="The project name for the selenium tests.",
)
...
args.build_tags = (
    [tag.strip() for tag in args.build_tags.split(",")] if args.build_tags else []
)
args.test_tags = (
    [tag.strip() for tag in args.test_tags.split(",")] if args.test_tags else []
)
...
class FirstSampleTest(unittest.TestCase):
    ...
    def run_demo_site_test(
        ...
        try:
            ...
            # Construct hub url
            base_url = (
                f"{args.hub_url}.lambdatest.com"
                if args.env == "prod"
                else f"stage-{args.hub_url}.lambdatestinternal.com"
            )
            ...
            if command_count > 0:
                ...
                for i in range(command_count):
                    driver.find_element(By.ID, "sampletodotext").send_keys(
                        f"LambdaTest {i}"
                    )
            ...
            errorFunctions = [
                ...
                lambda: (
                    ...
                    .move_to_element_with_offset(
                        driver.find_element(By.TAG_NAME, "body"), 9999999, 9999999
                    )
                    .perform(),
                ),
                ...
                lambda: (
                    driver.set_page_load_timeout(30),
                    driver.get("http://www.google.com"),
                    driver.find_element(By.CLASS_NAME, "gLFyf").send_keys("LambdaTest"),
                    driver.find_element(By.XPATH, "(//input[@name='btnK'])[2]").click(),
                ...
                lambda: (
                    ...
                    driver.set_page_load_timeout(30),
                    driver.get("http://www.google.com"),
                    driver.find_element(By.NAME, "btnI").clear(),
                ),  # InvalidElementState
                lambda: (
                    driver.get("http://www.google.com"),
                    driver.get("http://www.youtube.com"),
            ...
            if heading.is_displayed():
                ...
                if test_id < 2:
                    ...
                    try:
                        ...
                        logger.error(
                            "Element not found, moving on to quit the browser."
                        )  # This command will fail, making the test flaky
                elif 6 <= test_id < 8:
                    try:
                        driver.find_element(By.ID, "addbutton2")
                        # driver.execute_script("lambda-status=passed")
